...
class OxML_FullImage_Supervised_BreaKHis_v1_Dataset(Dataset):
    def __init__(self, config, set_name, this_transforms=None):
        ...
        if len(self.data_filenames)>0:
            self.images = torch.cat([self._pad_image(read_image(f[:-1])).unsqueeze(dim=0) for f in self.data_filenames], dim=0)
        else:
...
class OxML_FullImage_Supervised_BreaKHis_v1_Dataloader():
